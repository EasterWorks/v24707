1) The very first thing you will need to do is connect the printer to your own network. If you do not have an ethernet drop available for the printer, then you'll need to edit the wpa-supplicant.conf file on the Raspberry Pi's MicroSD card via the following:
...
2) After connecting the printer to your network, you will need to find the IP address.
...
3) The Raspberry Pi installed in this printer has the default password "raspberry". You should change this as soon as you're able to navigate to the printer's web console via the following:
